# All_with_Power_Limit_MILP - the MILP solution for three types of loads with power limit at each time step
# Input:
#   For All:
...
def All_with_Power_Limit_MILP(dt, pr, P_limit, L, P_NL, P_IL, E_IL, T_off, Pmin, P_TCL, c_water, m, temp_up, temp_o,
                              temp_req, temp_en, di):
    ...
    print('********************* Result *********************')
    print('NL Result: ', P_NL * result[0:N])
    print('IL Result: ', result[N:2 * N])
    print('TCL Result: ', result[6 * N:7 * N])
    print('Actual Sum of Power: ', P_NL * result[0:N] + result[N:2 * N] + result[6 * N:7 * N])
    print('Power Limit: ', P_limit)
