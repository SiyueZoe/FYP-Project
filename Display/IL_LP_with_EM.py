# IL_LP_with_EM - the LP solution for the Interruptable type load with Electrical Machinery
# Input:
...
#       Pmin - minimum power to turn on device
# Output:
#       F - the value of objective function [scalar]
#       x - the optimal schedule [vector]
#       y - the optimal scheduling ancillary binary which indicates the power status [vector]
#       z - the optimal scheduling variables to help in achieving constraints [vector]
...
def IL_LP_with_EM(dt, pr, P, E, T_off, Pmin):
    ...
    # A_eq - the array of ones to obtain total power consumption
    A_eq = np.concatenate((np.ones((1, N)), np.zeros((1, 4 * N))), axis=1)
    ...
    z_tmp2 = np.zeros((N, 3 * N))
    for i in range(N):
        z_tmp2[i, i * 3 + 1] = -Pmin
        z_tmp2[i, i * 3 + 2] = -P
    # tmp2 - coefficient to satisfy equality constraint: x - z2 * Pmin - z3 * P = 0
    tmp2 = np.concatenate((x_tmp, np.zeros((N, N)), z_tmp2), axis=1)
    # z_tmp3 - sub-vector of A_eq to obtain (z1 + z2 + z3) at each time step
    z_tmp3 = np.zeros((N, 3 * N))
    for i in range(N):
        z_tmp3[i, i * 3: (i + 1) * 3] = 1
    # tmp3 - coefficient to satisfy equality constraint: z1 + z2 + z3 = 1
    tmp3 = np.concatenate((np.zeros((N, 2 * N)), z_tmp3), axis=1)
    A_eq = np.concatenate((A_eq, tmp1, tmp2, tmp3))
    # b_eq - the vector for equality constraints
    b_eq = np.zeros((1, 1))
    b_eq[0, 0] = E / dt
    # b_eq_tmp1 - sub-vector of b_eq to satisfy equality constraints: y - z2 - z3 = 0 and x - z2 * Pmin - z3 * P = 0
    # b_eq_tmp2 - sub-vector of b_eq to satisfy equality constraints: z1 + z2 + z3 = 1
    b_eq_tmp1 = np.zeros((2 * N, 1))
    b_eq_tmp2 = np.ones((N, 1))
    ...
    A_ub = np.concatenate((A_ub, np.zeros(A_ub.shape)), axis=1)
    ...
    for i in range(T_off - 1):
        ...
        first_column = np.concatenate(([[1]], np.zeros((N - T_off - 1, 1))))
        first_row = np.concatenate(([1], [-1], np.zeros(i), [1], np.zeros(N - 3 - i)))
