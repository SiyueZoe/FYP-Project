# ES_NL - Non-interruptable load solved with exhaustive search
# Input:
#       dt - the time step [scalar]
#       pr - the array of prices [vector]
#       L - the duration of the task [vector]
#       P_NL - the power rate[vector]
#       NL_b - the beginning time [vector]
#       NL_e - the ending time [vector]
# Output:
#       np.sum(total_cost) - the total cost of all appliances throughout a day
...
def NL_ES(dt, pr, L, P_NL, NL_b, NL_e):
    ...
    N = len(pr)
    # an extremely large number waiting for the lower value to replace
