# All_MILP - the MILP solution for more loads(each type included) and time steps with demand response
# Input:
#   For All:
#       dt - the time step [scalar]
#       pr - the array of prices [array]
#       Energy_DR - demand response [scalar]
...
#       T_off - the minimum off time [array]
...
def All_MILP(pr, dt, Energy_DR, DR_b, DR_e, L, P_NL, NL_b, NL_e, P_IL, E_IL, T_off, Pmin, IL_b, IL_e, P_TCL, c_water,
    ...
    # <1> pr2 - expanded price array to fit array with variables to obtain objective function
    # <1.1> Expanded for NL
    pr2 = np.tile(pr, N_NL)
    tmp_NL = np.repeat(P_NL, N)
    pr2 = pr2 * tmp_NL
    # <1.2> Expanded for last two parts of IL
    tmp_IL = np.concatenate((pr, np.zeros(4 * N)))
    tmp_IL = np.tile(tmp_IL, N_IL)
    # <1.3> Expanded for TCL
    tmp_TCL = np.tile(pr, N_TCL)
    pr2 = np.concatenate((pr2, tmp_IL, tmp_TCL))
    # <2> DR_vector - array to help obtain total energy of all appliances within demand response time
    DR_vector = np.zeros(N_all * N)
    # <2.1> NL part in DR_vector
    for i in range(N_NL):
        for j in range(DR_b + i * N, DR_e + i * N + 1):
            DR_vector[j] = P_NL[i]
    # <2.2> IL part in DR_vector
    for i in range(N_IL):
        for j in range(DR_b + N * (N_NL + i * 5), DR_e + N * (N_NL + i * 5) + 1):
            DR_vector[j] = 1
    # <2.3> TCL part in DR_vector:
    for i in range(N_TCL):
    ...
    # All:
    # Demand Response
    m.addConstr(sum(x[i] * DR_vector[i] for i in range(N * N_all)) <= Energy_DR / dt)
    # NL:
    for i in range(N_NL):
