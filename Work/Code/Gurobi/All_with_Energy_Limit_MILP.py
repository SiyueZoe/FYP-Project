...
#       E_limit - the max amount of energy consumed by all loads throughout one day
#   For NL:
...
def ALL_with_Energy_Limit_MILP(dt, pr, E_limit, L, P_NL, P_IL, E_IL, T_off, Pmin, P_TCL, c_water, m, temp_up, temp_o,
                                  temp_req, temp_en, di):
    ...
    ## Add constraints
    # All: the total energy is within a limit
    m.addConstr(dt * sum(P_NL * x[i] + x[i + N] + x[i + 6 * N] for i in range(N)) <= E_limit)
    # NL:
    ...
    for v in m.getVars():
        result.append(v.x)
    for i in range(1, N + 1):
        lb_C.append(sum(C[j] for j in range(i)))
        ub_C.append(lb_C[i - 1] + C_limit)
        x_C.append(dt * sum(result[6 * N + j] for j in range(i)))
    lb_C = np.asarray(lb_C)
    ub_C = np.asarray(ub_C)
    x_C = np.asarray(x_C)
    ...
    print('Lower Bound: ', lb_C)
    print('Upper Bound: ', ub_C)
    print('Energy consumed by TCL: ', dt * sum(result[6 * N: 7 * N]))
    print('Total Cost: ', result[7 * N])

    return result[6 * N:7 * N], lb_C, ub_C, x_C
